...
/**
 *  {@link TDengineJDSLInterceptor},
 * 拦截所有 publish 消息,将消息内容和主题进行解析，根据规则存入TDengine
 *
 * @author huskar-t
 * @since 0.0.1
 */
public class TDengineJDSLInterceptor implements PublishInboundInterceptor {
    private final com.huskar_t.TDengineJDSL tdengine;
    private static final @NotNull Logger log = LoggerFactory.getLogger(com.huskar_t.TDengineJDSL.class);

    public TDengineJDSLInterceptor(com.huskar_t.TDengineJDSL tdengine) {
        ...
        final CompletableFuture<?> taskFuture = Services.extensionExecutorService().submit(() -> {
            ...
            try {
                ...
                if (payload.isPresent()) {
                    ...
                    tdengine.saveData(publishPacket.getTopic(), payloadStr);
                }
            } catch (Exception e) {
